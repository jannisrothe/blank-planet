...
export function createIslands(count, rand) {
  const spots = scatter(count, rand, 14, 30);
  const items = spots.map((s) => {
    const k = 5 + rand() * 16;
    return {
      x: s.x, z: s.z,
      // Well above cruising altitude: you fly under them, which is the look, and it
      // avoids the camera ending up inside one with no collision to stop it.
      y: heightAt(s.x, s.z) + 52 + rand() * 46,
